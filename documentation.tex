...
% Professional sidebar for "Ralph Observations"
\newmdenv[
  linecolor=zoho-blue,
  linewidth=2pt,
  leftmargin=10pt,
...
\newcommand{\ralph}[1]{
    \begin{ralphnote}
    \textit{\textbf{Observation:} #1}
    \end{ralphnote}
}

\title{Zoho OS: Minimal 64-bit Microkernel}
...
\begin{titlepage}
    ...
    \begin{mdframed}[
        ...
        \vspace{0.3cm}
        Create a minimal 64-bit kernel with core components like USB drivers, memory management, and a graphical shell.\par
        \vspace{0.5cm}
...
\ralph{I'm a system call! My glue tastes like memory management.}

\section{Project Philosophy}
Zoho OS is built to be a minimal yet extensible system. This requires:
\begin{itemize}
...
Zoho OS implements a **Modular Monolithic Architecture**. While all core services run within the same address space in Ring 0 for performance, the system is logically divided into strictly defined layers and modules to ensure maintainability and structural clarity.
...
\ralph{My architecture diagram looks like a city! I'm the mayor of the kernel.}
...
\ralph{I'm in danger! (If the GDT isn't loaded correctly).}
...
\ralph{Paging is like a sandwich. The physical frames are the bread, and the virtual addresses are the jelly!}

\section{Virtual Memory Manager (VMM)}
Zoho OS implements a 4-level paging system.
\begin{center}
...
Zoho OS manages hardware asynchronous events through a centralized Interrupt Descriptor Table (IDT).
\begin{center}
...
\end{tikzpicture}
\end{center}
\ralph{The interrupt button is like a doorbell. Ding-dong! Someone is at the CPU's house!}
...
\end{itemize}
\ralph{Files are like toys in a box. I have to open the box to play with them!}

\section{Observability (KStats \& KTrace)}
Zoho OS includes built-in observability tools that track kernel performance from the earliest boot stages.
\begin{itemize}
...
\end{itemize}
\ralph{I'm a spy! I'm watching every single byte that moves in this kernel.}
...
\ralph{I'm talking to two people at once! One is on the screen, and the other is in the wires.}

\section{USB Support (XHCI)}
Zoho OS features an eXtensible Host Controller Interface (XHCI) driver foundation, enabling high-speed communication with USB 3.0 devices such as human interface devices (HID) and mass storage.
\ralph{USB stands for Universal Super Best-friend!}

\chapter{Unique Features}
\section{Interactive Shell}
Zoho OS features a userland shell that supports basic file operations and system queries.
\ralph{I'm a shell! I'm the protective layer of the kernel egg.}

\section{Video Demonstration}
A full demonstration of the Zoho OS boot process, kernel initialization, and interactive shell is available in the project repository.
\begin{center}
    \includemedia[
        width=0.8\linewidth,
        height=0.45\linewidth,
        activate=onclick,
        addresource=docs/video/zoho.mp4,
        flashvars={
            source=docs/video/zoho.mp4
            &autoPlay=true
        }
    ]{
        \fbox{
            \parbox[c][6cm][c]{10cm}{
                \centering \Huge CLICK TO PLAY VIDEO
            }
        }
    }{VPlayer.swf}
\end{center}
\ralph{I'm a movie star! I hope I win an Oscar for "Best Kernel in a Leading Role".}
...
\ralph{I can see my house from here! (No, wait, those are just registers).}
...
\ralph{My pixels were standing in the wrong line. Now they are holding hands correctly!}
...
\ralph{I'm a bug! I'm hiding in your page tables.}
...
\ralph{All the CPUs are talking at once. My head hurts!}
...
\end{itemize}
\ralph{When I grow up, I want to be a server OS!}
...
\ralph{The logs are like stories, but with more numbers!}
...
\centering
\vspace{2cm}
\textbf{Zoho OS Lead Developer}\\
\vspace{1cm}
\href{mailto:atharva@example.com}{atharva@example.com}\\
\vspace{0.5cm}
\href{https://github.com/UnsettledAverage73/zoho-os}{Project GitHub}\\
\vspace{0.2cm}
\href{https://kernel.org/}{Project Reference}\\

\vspace{3cm}
\ralph{I'm a documentation! I helped you get a job.}
