...
The PMM uses a stack-based frame allocator combined with a bitmap for high-performance memory tracking.

\begin{center}
\begin{tikzpicture}[node distance=1.5cm]
    \draw[thick] (0,0) rectangle (8,1) node[midway] {Physical Memory (RAM)};
    \foreach \x in {0.5,1.5,2.5,3.5,4.5,5.5,6.5,7.5}
        \draw (\x,0) -- (\x,1);
    
    \node (center) at (4,0.5) {};
    \node (stack) [draw, fill=zoho-blue!20, minimum width=2cm, minimum height=3cm, below=of center] {Free Frame Stack};
    \draw[->, ultra thick, zoho-blue] (stack.north) -- (4,0) node[midway, right] {Push/Pop};
    
    \node (bitmap) [draw, fill=zoho-green!20, minimum width=3cm, minimum height=0.8cm, right=of stack] {Bitmap Tracker};
    \draw[<->, dashed] (bitmap.west) -- (stack.east) node[midway, above, font=\tiny] {Sync};
    
    \node at (4,-4) {\textbf{PMM Frame Allocation Mechanics}};
\end{tikzpicture}
\end{center}
